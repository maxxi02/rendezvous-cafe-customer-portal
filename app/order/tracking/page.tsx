...
function TrackingContent() {
  ...
  const [currentStatus, setCurrentStatus] =
    useState<QueueStatus>("pending_payment");
  ...
  useEffect(() => {
    ...
    const handleOrderGetResult = (order: any) => {
    ...
    const handleOrderPaymentSuccess = (data: {
      orderId: string;
      queueStatus: QueueStatus;
    }) => {
      if (data.orderId === orderId) {
    ...
    socket.on("order:get:result", handleOrderGetResult);
    socket.on("order:status:changed", handleOrderStatusChanged);
    socket.on("order:payment:success", handleOrderPaymentSuccess);

    return () => {
      socket.off("order:get:result", handleOrderGetResult);
      socket.off("order:status:changed", handleOrderStatusChanged);
      socket.off("order:payment:success", handleOrderPaymentSuccess);
    };
  }, [orderId, socket]);
