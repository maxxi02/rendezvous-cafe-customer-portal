...
export function AuthModal({ isOpen, onClose, callbackURL, title, message }: AuthModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-sm" hideClose>
      <div className="flex flex-col items-center text-center py-2">
        ...
        <div className="flex items-center gap-2 mb-8">
          <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center shadow-lg shadow-primary/30">
            <Coffee size={18} className="text-primary-foreground" strokeWidth={2.5} />
          </div>
        ...
        <button
          onClick={async () => {
            await signIn.social({ provider: "google", callbackURL: currentCallback });
          }}
