...
import { useBranding, DEFAULT_BRANDING } from "@/app/providers/BrandingProvider";
...
function NavActions({
    ...
    return (
      <div className="flex items-center gap-3">
        <Image
          src={
            session.user.image ??
            `https://ui-avatars.com/api/?name=${encodeURIComponent(session.user.name ?? "U")}&background=064e3b&color=fbbf24`
          }
  ...
  return (
    <button
      onClick={openAuth}
      className="hidden md:block px-5 py-2.5 rounded-full font-black text-xs tracking-widest uppercase transition-all duration-300"
...
function MobileNavActions({
  ...
  if (isPending)
    return <div className="w-full h-12 rounded-full bg-white/10 animate-pulse mt-4" />;
  ...
  return (
    <button
      onClick={openAuth}
      className="border border-white/20 text-white/80 px-6 py-3 rounded-full font-black text-sm tracking-widest uppercase text-center transition-all duration-300 mt-4"
...
function NavbarInner() {
  ...
  const { primaryColor, accentColor, logoText, logoUrl } = branding;
    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 px-6 py-4 md:px-12 transition-all duration-500 ${
          scrolled ? "backdrop-blur-md shadow-lg shadow-black/30" : ""
        }`}
        style={{
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2">
            {logoUrl ? (
              ...
              /* Text logo — font controlled via --brand-logo-font CSS var */
              <span
                className="text-2xl tracking-wider text-white transition-all duration-300"
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`relative pb-1 transition-colors duration-300 group ${
                  pathname === link.href ? "" : "text-white/75"
                }`}
          ...
          <div className="flex items-center gap-3">
            ...
            <NavActions openAuth={openAuth} primary={primaryColor} />
      ...
      >
        ...
        />
        <div
          className={`absolute top-0 right-0 h-full w-72 bg-background flex flex-col pt-24 px-8 gap-6 shadow-2xl transition-transform duration-500 ${
        ...
        >
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={() => setMobileOpen(false)}
              className="text-2xl font-black uppercase tracking-widest transition-colors duration-300 border-b border-white/10 pb-4"
              style={{ color: pathname === link.href ? primaryColor : "white" }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.color = primaryColor; }}
              onMouseLeave={(e) => {
                if (pathname !== link.href)
